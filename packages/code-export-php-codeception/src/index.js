...
function generateTestDeclaration(name) {
  name = exporter.parsers.capitalize(name)
...
function generateSuiteComment() {
  return `/**\n/* created by Selenium IDE\n*/\n`
}

function generateSuiteDeclaration(name) {
  name = exporter.parsers.capitalize(name)
  return `class ${name}Cest {`
}

function generateFilename(name) {
  name = exporter.parsers.capitalize(name)
  return `${name}${opts.fileExtension}`
}
...
export async function emitTest({
  ...
  const suiteComment = generateSuiteComment()
